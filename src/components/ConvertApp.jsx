...
function ConvertApp() {
  ...
  const [txValue, setTxValue] = useState(new BigNumber(850).dividedToIntegerBy(AVG_TX_FEE_USD))
  useEffect(() => {
    setTxValue(usdValue ? new BigNumber(usdValue).dividedToIntegerBy(AVG_TX_FEE_USD) : 0)
  }, [usdValue])
  const [reason, setReason] = useState('')

  const generateTweet = () => `I spent a total of ${txValue} Solana transactions${reason ? ` for ${reason}.` : '.'}%0A%0A%23OnlyPossibleOnSolana%0A%0AFind out how much you would have spent at https://www.solfees.fyi/convert by %40solfees_fyi`

  return (
    <main className="min-h-screen flex flex-col items-center justify-center">
      <FadeInOutTransition show={appReady} className="flex flex-col items-center justify-center gap-1 max-w-xs">
        <label>USD</label>
        <Input
          value={usdValue}
